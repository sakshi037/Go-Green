...
const btnCloseModalWishlist = document.querySelector('.btn--close-modal-wishlist');
const btnCloseModalCart = document.querySelector('.btn--close-modal-cart');
const btnCloseModalPlaceOrder = document.querySelector('.btn--close-modal-placeorder');
...
const wishlist = function(e){
 ...
 {
      if(wishlistname1.textContent===productName.textContent||wishlistname3.textContent===productName.textContent
        ||wishlistname2.textContent===productName.textContent)
      {
      alert("This Item is Already Present in Wishlist🤩")
      }
 ...
 else if(wishlistname1.textContent!=""&&wishlistname2.textContent!=""&&wishlistname3.textContent!=""&&
...
const addToBag = function(e){
  ...
  {
       if(cartname1.textContent===productName.textContent||cartname3.textContent===productName.textContent
         ||cartname2.textContent===productName.textContent)
       {
       alert("This Item is Already Present in Your Bag🤩")
       }
   ...
   //wishlist block----------no block available wishlist is full
  else if(cartname1.textContent!=""&&cartname2.textContent!=""&&cartname3.textContent!=""&&
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////
//Search button
let arr=["AGLONEEMA","ALGONEEMACOMMUTATUMPLANT","PURPLE",
"PURPLEFLOWER","SNAKE","SNAKEPLANT","ASPIDISTRA","ASPIDISTRAPLANT",
"ALOVERA","ALOVERAPLANT","CACTUS","CACTUSPLANT","LILY","LILYFLOWER","SYNGONIUM",
"SYNGONIUMPLANT","MONEYPLANT","JADE","JADEPLANT","CRASULLAEROSULLACAMFIRE","YUCCA",
"YUCCA PLANT","BOUGAINVILLEA","BOUGAINVILLEAPLANT","AGAVE","AGAVEPLANT",
"SWISSCHEESE","SWISSCHEESEPLANT","CATARACTRUM","CATARACTRUMPLANT"];
...
btnsLoginModal.forEach(btn => btn.addEventListener('click', openModal.bind(modalLogin)))
btnCloseModal.addEventListener('click', closeModal.bind(modalLogin));
overlay.addEventListener('click', closeModal.bind(modalLogin));

btnsWishlistModal.forEach(btn => btn.addEventListener('click', openModal.bind(modalWishlist)))
...
btnsCartModal.forEach(btn => btn.addEventListener('click', openModal.bind(modalCart)))
...
btnsPlaceOrder.addEventListener('click', function(){
  if(cartname1.textContent!=""||cartname2.textContent!=""||cartname3.textContent!=""||cartname4.textContent!="")
  {
       openModal2();
  }
  else{
...
btnCloseModalPlaceOrder.addEventListener('click', closeModal2.bind(modalPlaceOrder));
...
const orderTimer = function(){
  const tick = function(){
    ...
    document.getElementById("timerfororder").textContent = `${hour}:${min}:${sec}`;
    if(time===0)
    {
      clearInterval(timer);
    }
...
//seed Window
btnsSeedModal.forEach(btn => btn.addEventListener('click', openModal1.bind(modalSeed)));

overlay1.addEventListener('click', closeModal1.bind(modalSeed));
//plant Window
btnsPlantModal.forEach(btn => btn.addEventListener('click', openModal1.bind(modalPlant)));

overlay1.addEventListener('click', closeModal1.bind(modalPlant));
//pack window
btnsPackModal.forEach(btn => btn.addEventListener('click', openModal1.bind(modalPack)));

overlay1.addEventListener('click', closeModal1.bind(modalPack));
//soil window
btnsSoilModal.forEach(btn => btn.addEventListener('click', openModal1.bind(modalSoil)));

overlay1.addEventListener('click', closeModal1.bind(modalSoil));
//pebbles window
btnsPebbleModal.forEach(btn => btn.addEventListener('click', openModal1.bind(modalPebble)));

overlay1.addEventListener('click', closeModal1.bind(modalPebble));
//tools window
btnsToolModal.forEach(btn => btn.addEventListener('click', openModal1.bind(modalTool)));

overlay1.addEventListener('click', closeModal1.bind(modalTool));
//flower window
btnsFlowerModal.forEach(btn => btn.addEventListener('click', openModal1.bind(modalFlower)));
...
const navHeight = nav1.getBoundingClientRect().height;

const stickyNav = function (entries) {
  const [entry] = entries;
  // console.log(entry);

  if (!entry.isIntersecting) nav1.classList.add('sticky');
